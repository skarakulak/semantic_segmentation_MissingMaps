...
class U_Net(object):
    ...
    def dataGenerator(self,trainingBatch=True):
        ...
        if(trainingBatch):
            ...
            self.train_x_new = np.true_divide(np.array(
                [augmentImage(imageio.imread(datasetAddr+'/Train/{0}/tiles/{1}.jpg'.format(t[0],t[1])), augArgs)
                 for t, augArgs in 
                 zip(
            ...
            tmpLabels = np.array(
                [augmentImage(imageio.imread(datasetAddr+'/Train/{0}/labels/{1}.png'.format(t[0],t[1])), augArgs)
                 for t, augArgs in 
                 zip(
                    self.train_folderAndTiles[self.trainingDataIndex:self.trainingDataIndex+self.batchSize],
                    augParameters
            ...
            self.train_y_new =np.zeros_like(tmpLabels)
            for i,v in enumerate([0,17,204]):  # [0,17,204] for [background, buildings, roads]
                self.train_y_new[:,:,:,i] = (tmpLabels[:,:,:,0]==v).astype('float32')
...
#        mIOU_arr = np.zeros((evalNTimes,),dtype='float32')
    ...
    def getSpecificData(self, project,tileID, dataset,returnImg=False):
        datasetAddr = self.datasetAddr
        if(dataset=='training'):
            pAddr = datasetAddr+'/Train/'
        elif(dataset=='validation'):
            pAddr = datasetAddr+'/Validation/'
        elif(dataset=='validation_random'):
            pAddr = datasetAddr+'/Validation_Randomized/'
        elif(dataset=='test'):
            pAddr = datasetAddr+'/Test/'
        elif(dataset=='test_random'):
            pAddr = datasetAddr+'/Test_Randomized/'

        arr_x = np.array(
            imageio.imread(pAddr +'{0}/tiles/{1}.jpg'.format(str(project),str(tileID))))
        arr_x_norm = np.true_divide(arr_x,127.5) - 1.

        tmpLabels = np.array(
            imageio.imread(pAddr + '{0}/labels/{1}.png'.format(str(project),str(tileID))))

        arr_y =np.zeros_like(tmpLabels)
        for i,v in enumerate([0,17,204]):
            arr_y = (tmpLabels[:,:,0]==v).astype('float32')

        if (returnImg):
            return(arr_x, arr_x_norm, arr_y)
        else:
            return(arr_x, arr_x_norm, arr_y)

    def drawPreds(self,sess):
        tilesInReport = [
            ('18_4195','158093-131842-18','training',True),
            ('17_3649','76183-49158-17','training',True),
            ('18_3994','151663-142389-18','test',True),
            ('18_5370','159704-136176-18','training',True)
        ]
        for t in tilesInReport:
            arr_x, arr_x_norm, arr_y = self.getSpecificData(t[0],t[1], t[2],t[3])
            pred_y = sess.run(
                [self.preds],
                feed_dict={self.x_ph: arr_x_norm[None,:,:,:]}
                )

            pred_y = (pred_y[0][0,:,:,:]).reshape((256*256,3))
            temp_y = np.zeros_like(pred_y)
            temp_y[np.arange(len(pred_y)), pred_y.argmax(1)] = 1
            temp_y = temp_y.reshape((256,256,3))
            
            plt.imsave('imgs/'+t[1]+'_pred.png', temp_y)
